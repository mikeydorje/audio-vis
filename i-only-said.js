// --- I Only Said: Crystalline Scatter ---
// Sparse irregular 3D grid — like lattice but with wider spacing
// and huge soft halos. Cold, precise geometry dissolving into light.
// Ice blue whispers, frosted edges, brittle echoes.
...
const vertexShader = `
  ...
  void main() {
    ...
    gl_PointSize = aSize * (5.0 + gAmp * 5.0 + uWhisper * 2.0 + gAmp2 * 1.0) * (300.0 / -mvPos.z);
...
const fragmentShader = `
  ...
  void main() {
    ...
    // Fine frost grain
    float grain = hash(gl_FragCoord.xy + uTime * 2.5) * uHiss * 0.05;
    color += grain;

    alpha *= 0.12 + vFreqAmp * 0.3 + vCrystal * 0.1;
...
// --- Sparse irregular 3D grid (lattice-style, wide spacing) ---
let particles;
...
const clock = new THREE.Clock(), DRIFT_BASE = 280;
...
window.SCENE = {
  ...
  get playState() { return playState; }, sceneName: 'i-only-said'
};
animate();
