// --- τ (Tau): Audio-Reactive Spirograph Morphing ---
// τ = 2π = full turn. Particles trace epicycloid/hypotrochoid curves.
// Silence = perfect circles. Sound = Spirograph chaos.
...
const vertexShader = `
  ...
  void main() {
    ...
    // --- Hypotrochoid: circle rolling INSIDE (Spirograph) ---
...
function initAudio(sm) {
  ...
  analyser = audioContext.createAnalyser(); analyser.fftSize = 128; analyser.smoothingTimeConstant = sm;
...
// Narrative: Circle → Winding → Spirograph → Harmony → Return
function storyArc(p) {
...
function animate() {
  ...
  for (const k in driftCycles) {
    ...
    const d = (Math.sin(elapsed * TP / period) * 0.65 + Math.sin(elapsed * TP / (period * 2.17) + 1.3) * 0.35) * sd;
  ...
  particles.rotation.x = elapsed * rotSpeedX * 0.4 * (arc.rot||1);
...
window.SCENE = {
  ...
  uniformMap: uMap,
  rotXMult: 0.4, rotDriftScale: 0, tiltDriftScale: 0,
  storyArc,
