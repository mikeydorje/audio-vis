// --- Only Shallow: Feedback Saturation Field ---
// No center. Soft blobs scattered through a volume, warping and
// bleeding into each other. Long-exposure colour fields that phase
// through tape saturation. White-hot gradients dissolving into crimson.
...
const vertexShader = `
  ...
  void main() {
    ...
    gl_PointSize = aSize * (3.5 + gAmp * 5.0 + uSaturation * 2.0 + trem * 1.5) * (300.0 / -mvPos.z);
...
const fragmentShader = `
  ...
  void main() {
    ...
    // Film grain
    float grain = hash(gl_FragCoord.xy + uTime * 4.0) * uHiss * 0.1;
    color += grain;

    alpha *= 0.2 + vFreqAmp * 0.4 + vHeat * 0.15;
...
function computeSeedValues() {
  ...
  return {
    ...
    rotSpeedY: 0.003 + feedback * 0.012, rotSpeedX: 0.002 + feedback * 0.006, smoothing: 0.96 - tremolo * 0.15, detail: Math.floor(8 + tremolo * 24),
    hiss: sliders.p5.value / 100, epoch: sliders.p6.value / 100, threshold: sliders.p7.value / 100, flux: sliders.p8.value / 100
  };
}

// --- Decentralized blob field (no center reference) ---
let particles;
...
window.SCENE = {
  ...
  rotXMult: 0.03, rotDriftScale: 0, tiltDriftScale: 0, storyArc,
  ...
  get playState() { return playState; }, sceneName: 'only-shallow'
};
animate();
