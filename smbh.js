// --- Three.js Audio-Reactive Visualizer ---

// Vertex shader: displaces particles as waves driven by audio
const vertexShader = `
...
// Fragment shader: granular spectral color from frequency data
...
// --- Seed Parameters (nature controls) ---
const sliders = {
...
function computeSeedValues() {
  ...
  const rhizome = sliders.rhizome.value / 100;
...
// --- Baked seed centers (for drift modulation) ---
let rotSpeedY = 0.15;
let rotSpeedX = 0.1;
...
// Load persisted audio on startup
...
// --- Narrative Arc ---
// Maps song progress (0→1) to per-param multipliers that tell a story
...
function storyArc(progress) {
  ...
  // Each param gets its own narrative shape
  // bloomAmp: how explosively it reacts
  const bloomArc = 0.3
    + 0.3 * sm(0.0, 0.15, p)    // wake
    + 0.5 * sm(0.15, 0.45, p)   // build
    - 0.2 * sm(0.65, 0.85, p)   // soften
    - 0.5 * sm(0.85, 1.0, p);   // fade

  // waveMag: height of surface waves
  const waveMagArc = 0.2
    + 0.2 * sm(0.05, 0.2, p)    // gentle stir
    + 0.7 * sm(0.25, 0.55, p)   // ocean swells
    - 0.3 * sm(0.6, 0.8, p)    // calming
    - 0.5 * sm(0.85, 1.0, p);   // still

  // waveFreq: spatial density — tight at climax, open at bookends
  const waveFreqArc = 0.6
    + 0.15 * sm(0.1, 0.3, p)
    + 0.4 * sm(0.3, 0.5, p)     // dense weave
    - 0.25 * sm(0.65, 0.85, p)
    - 0.3 * sm(0.88, 1.0, p);

  // waveSpeed: tempo — slow open, fast middle, lingering close
  const waveSpeedArc = 0.4
    + 0.25 * sm(0.1, 0.25, p)
    + 0.5 * sm(0.25, 0.5, p)    // rushing
    - 0.15 * sm(0.55, 0.7, p)   // catching breath
    + 0.15 * sm(0.7, 0.8, p)    // second wind
    - 0.6 * sm(0.82, 1.0, p);   // exhale

  // freqSpread: how much of the spectrum is visible — narrow→wide→narrow
  const freqSpreadArc = 0.5
    + 0.15 * sm(0.05, 0.2, p)
    + 0.4 * sm(0.2, 0.5, p)     // full spectrum
    - 0.1 * sm(0.6, 0.75, p)
    - 0.4 * sm(0.8, 1.0, p);    // narrowing to silence

  // Rotation: slow contemplation → spinning energy → gentle fade
  const rotArc = 0.5
    + 0.2 * sm(0.1, 0.3, p)
    + 0.5 * sm(0.3, 0.55, p)
    - 0.3 * sm(0.7, 0.9, p)
...
function animate() {
  ...
  // --- Song narrative arc (shaped by Mycelium) ---
  ...
  // --- Slow gradient drift: each param breathes on its own cycle ---
  ...
  // Rotation drift: narrative arc shapes the speed envelope
  const rotDrift = Math.sin(elapsed * TWO_PI / (DRIFT_BASE * 0.92)) * 0.15;
  const tiltDrift = Math.sin(elapsed * TWO_PI / (DRIFT_BASE * 1.38) + 2.0) * 0.12;

  // Read frequency data if audio is playing
  ...
  // Rotate the particle field — arc + drift shape the journey
