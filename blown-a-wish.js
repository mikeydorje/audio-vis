// --- Blown A Wish: Dispersal Field ---
// Cluster-based scattered outward — wisps drifting apart like
// dandelion seeds. Breeze carries them, lift pulls them skyward.
// Pale sage, white wisps, soft gold catch-light. Maximum airiness.
...
const vertexShader = `
  ...
  void main() {
    ...
    float outward = sin(uTime * 0.008 + aPhase * 2.0) * uScatter * 0.2;
    float outwardY = cos(uTime * 0.006 + aNode * 3.0) * uScatter * 0.12;

    // Breeze: lateral drift — wind carrying wisps sideways
    float wind = sin(uTime * 0.01 + aTrajectory * 5.0 + aNode * 2.0) * uBreeze * 0.18;
    ...
    gl_PointSize = aSize * (4.5 + gAmp * 5.0 + uEphemeral * 2.5 + gAmp2 * 1.5) * (300.0 / -mvPos.z);
...
const fragmentShader = `
  ...
  void main() {
    ...
    float grain = hash(gl_FragCoord.xy + uTime * 2.0) * uHiss * 0.03;
    color += grain;

    alpha *= 0.12 + vFreqAmp * 0.3 + vWisp * 0.1;
...
function computeSeedValues() {
  ...
  return {
    ...
    rotSpeedY: 0.002 + breeze * 0.006, rotSpeedX: 0.001 + breeze * 0.003, smoothing: 0.97 - scatter * 0.1, detail: Math.floor(5 + scatter * 6),
    hiss: sliders.p5.value / 100, epoch: sliders.p6.value / 100, threshold: sliders.p7.value / 100, flux: sliders.p8.value / 100
  };
}

// --- Dispersal clusters: scattered outward from origins ---
let particles;
function buildParticles(detail) {
  ...
  // Origins scattered across the volume — no center
  const origins = [];
  ...
  // Particles scattered from origins — blown outward
  for (let o = 0; o < numOrigins; o++) {
...
const clock = new THREE.Clock(), DRIFT_BASE = 270;
